...
export const metadata = createPageMetadata({ title: "Pitcher Map | MLB Pitcher Similarity", description: "Explore MLB pitcher similarity in model space using Statcast arsenal analysis, pitch profiles, and AXScout baseball intelligence.", path: "/pitchers/map" });
...
export default async function PitcherMapPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return <div className="space-y-8">
    ...
    {result.data.length ? <>
      ...
      {points.length ? <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold text-slate-900">Similarity space</h2><p className="mt-1 text-sm text-slate-500">{points.length} pitchers with stored map coordinates</p></div><p className="text-xs text-slate-500">Hover for details · click a point for its profile</p></div><PitcherMapChart points={points} /></section> : <div className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm"><h2 className="text-lg font-semibold text-slate-900">No pitchers match these map filters</h2><p className="mt-2 text-sm leading-6 text-slate-600">Adjust the season, archetype, role, or search filters.</p></div>}
    </> : <section className="rounded-xl border border-dashed border-blue-200 bg-gradient-to-br from-blue-50 to-white p-8 text-center shadow-sm sm:p-12"><p className="text-xs font-semibold uppercase tracking-wide text-blue-700">Similarity visualization</p><h2 className="mt-2 text-2xl font-bold text-slate-950">Pitcher Map is coming soon.</h2><p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-slate-600">This view will plot pitchers in model similarity space so nearby pitchers have similar arsenal and pitch-profile characteristics. The X and Y axes represent model similarity space and should not be interpreted as direct baseball metrics.</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Link href="/pitchers" className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">Back to Pitcher Explorer</Link><Link href="/pitchers/archetypes" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:border-blue-300 hover:text-blue-700">View Archetype Library</Link></div></section>}
