...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createPageMetadata({ title: `${name} | MLB Pitcher Archetype`, description: `${name} defining features, representative pitchers, matchup context, and Statcast arsenal similarity analysis from AXScout baseball intelligence.`, path: `/pitchers/archetypes/${slug}` });
}
...
export default async function PitcherArchetypePage({ params, searchParams }: Props) {
  ...
  return <div className="space-y-8">
    ...
    <SectionCard title="Archetype Summary" description="A compact view of this group’s size, representative pitcher, and current model context."><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{[["Pitchers", archetype.pitcherCount.toString()], ["Representative", archetype.representativeName ?? "Unavailable"], ["Silhouette", archetype.silhouetteScore?.toFixed(3) ?? "—"], ["Model", archetype.modelVersion], ["Features", archetype.featureVersion]].map(([label, value]) => <div key={label} className="rounded-lg border border-slate-200 bg-slate-50 p-4"><p className="text-sm text-slate-500">{label}</p><p className="mt-1 break-words text-lg font-bold text-slate-950">{value}</p></div>)}</div></SectionCard>
    ...
    <SectionCard title="Model Notes" description="Archetype names may be model-generated placeholders unless manually reviewed."><div className="space-y-2 text-sm leading-6 text-slate-600"><p>Archetype labels can be manually reviewed and refined after inspecting representative pitchers and defining features.</p><p>Cluster membership describes similarity in the current standardized feature space, not a permanent scouting label. Model versions, feature inputs, and sample windows can change assignments over time.</p></div></SectionCard>
  </div>;
}
